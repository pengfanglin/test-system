...
public class IdCardUtils {
     ...
     * @return
     */
    public static boolean isPass(String idCard){
        if(OthersUtils.isEmpty(idCard)){
            throw new RuntimeException("身份证号不能为空");
        }
        return isIdNoPattern(idCard) && isValidProvinceId(idCard.substring(0, 2))
                && isValidDate(idCard.substring(6, 14)) && checkIdNoLastNum(idCard);
    }
    /**
     * 二代身份证正则表达式
     *
